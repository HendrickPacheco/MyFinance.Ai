...
export const CATALOGO_FERRAMENTAS: readonly FerramentaCatalogo[] = [
  ...
  },
  {
    ...
    descricao:
      'Decomposição da RENDA do ciclo atual em blocos que somam exatamente 100% dela: Custos fixos, Provisão mensal, Ajuste de rollover e Disponível para gastar — este último subdividido em "Parcelamentos do ciclo" e "Gastos eventuais do mês". Use para "para onde vai meu dinheiro", "quanto da minha renda é custo fixo", "quanto sobra depois de tudo". Diferente de gastos_por_categoria, que só olha gasto de verba variável: aqui entra TUDO, inclusive custo fixo e provisão. ATENÇÃO: "Parcelamentos do ciclo" já está dentro de "Disponível para gastar" — somá-lo aos blocos de topo conta o mesmo dinheiro duas vezes. A meta de poupança NÃO é um bloco (D-16): ela não é descontada da renda, o dinheiro dela está dentro de "Disponível para gastar". Use os rótulos de rotulosParaODono; nunca diga "verba variável" nem "verba livre" ao dono. Quando misturaOrigens for true numa categoria, cite as partes por origem, nunca só o total.',
    argumentos: z.object({
      limite: z
        .number()
        .int()
